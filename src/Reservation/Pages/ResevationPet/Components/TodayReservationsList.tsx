...
//         Swal.fire({
//           title: "Error",
//           text: err?.response?.data?.message || "Failed to archive reservation",
//           icon: "error",
//         });
//       },
//     });
...
const TodayReservationsList: React.FC = () => {
  ...
  // normalize reservations (support { data: [] } or array)
  const reservations = useMemo(() => (allReservations?.data ?? allReservations) ?? [], [allReservations]);

  const { page, totalPages, paginatedItems, goToPage } = useLocalPagination(reservations, 5);
  ...
  const handleSoftDelete = async (id: string) => {
    ...
    softDelete(id, {
      onSuccess: () => {
        Swal.close();
        Swal.fire({ title: "Archived", text: "Reservation archived successfully", icon: "success", timer: 1400, showConfirmButton: false });
      },
      onError: (err: any) => {
        Swal.close();
        Swal.fire({ title: "Error", text: err?.response?.data?.message || "Failed to archive reservation", icon: "error" });
      },
    });
  };
  ...
  const handleHardDelete = async (id: string) => {
    ...
    hardDelete(id, {
      onSuccess: () => {
        Swal.close();
        Swal.fire({ title: "Deleted", text: "Reservation deleted successfully", icon: "success", timer: 1400, showConfirmButton: false });
      },
      onError: (err: any) => {
        Swal.close();
        Swal.fire({ title: "Error", text: err?.response?.data?.message || "Failed to delete reservation", icon: "error" });
      },
    });
  };
  ...
  return (
    <>
      <ViewReservationModal isOpen={viewOpen} onClose={() => setViewOpen(false)} reservation={activeReservation} />
        ...
        {paginatedItems.map((res: any, i: number) => (
          ...
          >
            <div className="flex gap-4 items-start">
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-3">
                  <h2 className="text-[18px] font-semibold text-[#4A3F35]">{res.pet?.name}</h2>
                ...
                <p className="text-sm text-[#8C827A]">{res.petOwner?.fullName ?? res.petOwner?.userName}</p>

                <div className="flex items-center gap-3 mt-1 text-sm">
                  <Clock size={15} className="text-[#A39A90]" />
                  <span className="font-medium text-[#4A3F35]">{res.timeSlot}</span>
                  <span className="text-[#D29A84] font-medium">{res.service?.name}</span>
                </div>
              </div>
            </div>

            <div className="flex items-center gap-3">
              <button onClick={() => openView(res)} className="p-1 rounded-full hover:bg-gray-100 transition">
                <Eye size={18} className="text-[#6F665F]" />
              </button>

              <button onClick={() => openEdit(res)} className="p-1 rounded-full hover:bg-gray-100 transition">
                <Edit size={18} className="text-amber-600" />
              </button>

              <button className="p-1 rounded-full hover:bg-gray-100 transition" onClick={() => handleSoftDelete(res._id)}>
                <FaArchive size={18} className="text-gray-600" />
              </button>

              <button className="p-1 rounded-full hover:bg-gray-100 transition" onClick={() => handleHardDelete(res._1d)}>
                <FaTrash size={18} className="text-red-700" />
              </button>
            </div>
          </motion.div>
