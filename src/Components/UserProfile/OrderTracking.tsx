...
const PetOrderTracking: React.FC = () => {
  ...
  // ===== Timeline Steps 🐾 =====
  const steps = [
    { key: orderStatus.PLACED, label: t("userProfile.tracking.steps.placed"), icon: <FaPaw /> },
    { key: orderStatus.SHIPPING, label: t("userProfile.tracking.steps.shipping"), icon: <FaTruck /> },
    { key: orderStatus.COMPLETED, label: t("userProfile.tracking.steps.completed"), icon: <FaCheckCircle /> },
    { key: orderStatus.CANCELED, label: t("userProfile.tracking.steps.canceled"), icon: <FaTimesCircle /> },
    { key: orderStatus.REFUNDED, label: t("userProfile.tracking.steps.refund"), icon: <FaUndo /> },
  ];
    ...
    <>
      ...
      <div className="max-w-4xl mx-auto mt-12 p-8 rounded-3xl bg-[var(--color-bg-lighter)] dark:bg-[var(--color-dark-card)] shadow-xl border border-[var(--color-border-light)] dark:border-[var(--color-dark-border-light)] transition-colors duration-300">
        ...
        >
          ...
          <button
            type="submit"
            className="bg-[#8B5E35] dark:bg-[var(--color-dark-accent)] text-white px-6 py-2 rounded-xl hover:bg-[#734E2F] dark:hover:bg-[var(--color-dark-accent)]/80 transition shadow-md font-bold"
          >
          ...
          <>
              ...
              <div className="flex justify-between">
                {steps.map((step, index) => (
                  ...
                  >
                    ...
                    <p
                      className={`mt-3 text-sm font-semibold text-center ${index <= stepIndex
            ...
            <div className="bg-[var(--color-bg-cream)] dark:bg-[var(--color-dark-background)] backdrop-blur-lg shadow-lg rounded-2xl border border-[var(--color-border-light)] dark:border-[var(--color-dark-border-light)] p-6">
              <h3 className="text-xl font-bold text-[#8B5E35] dark:text-[var(--color-dark-accent)] mb-3">
                {t("userProfile.orders.idPrefix")}{order._id.slice(-6).toUpperCase()}
              </h3>
