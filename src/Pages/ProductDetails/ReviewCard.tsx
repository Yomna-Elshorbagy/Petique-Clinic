...
export default function ReviewCard({ reviews, users, onDelete }: Props) {
  ...
  return (
    <>
      ...
      >
        {reviews.map((review) => {
          const user = users?.find((user) => user._id === review.user._id);
          if (!user) return null;
          ...
          return (
            ...
            >
              <button
                ...
                className="absolute top-5 right-3 text-red-500 hover:text-red-600  z-20"
              >
                <MdDelete className="text-xl" />
              </button>
              <div className="flex items-center gap-3 mb-3">
                <img
                  src={user?.image.secure_url || "/default-avatar.png"}
                  alt={user?.userName}
              ...
              <p className="text-gray-700 leading-relaxed ">
                {expandedId[review._id]
                  ? review.comment
                  : review.comment.split(" ").slice(0, Max_Length).join(" ")}
                {review.comment.split(" ").length > Max_Length && (
                  <span
