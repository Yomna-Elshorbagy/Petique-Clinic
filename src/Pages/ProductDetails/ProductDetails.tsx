...
import type { productdetails, review, Users } from '../../Types/ProductDetailsType';
...
export default function ProductDetails() {
  ...
  let { isLoading: relatedLoading, isError: relatedError, data: relatedProducts } = useQuery<productdetails[]>({
    queryKey: ['relatedproducts', id],
    queryFn: getrelatedproducts
  });
// get review
  async function getreviews(){
    const {data: reviews} =await axios.get(`${baseURL}/reviews/productReviews/${id}`,{
      headers:{
 ...
 let { isLoading: reviewloading, isError: reviewError, data:reviewdata } = useQuery<review[]>({
    queryKey: ['Reviews', id],
    queryFn: getreviews
  });
  // get user 
 ...
 const{data:userdata}= useQuery<Users[]>({
  ...
  const addreview = useMutation({
    mutationFn: async(newReview:{rate:number;comment:string})=>{
     const {data} =await axios.post(`${baseURL}/reviews/`,{
        product:id,
        rate:newReview.rate,
    ...
    onSuccess:()=>{
       toast.success('Review added successfully', { position: 'top-center', autoClose: 1000 });
      queryClient.invalidateQueries({ queryKey: ['Reviews', id] });
    },
    onError:(error)=>{
        ...
        <div className="max-w-6xl mx-auto pt-28">
          ...
          <div className="flex flex-col md:flex-row  gap-10 items-center md:items-start  md:justify-between">
            ...
            <div className="flex flex-col items-center">
              ...
              ${selectedImage === img.secure_url ? 'border-[#e9a66f] border-2' : 'border-gray-300'}
            `}
            ...
            <div className="w-full  md:w-1/2 flex flex-col gap-4 bg-white/60 backdrop-blur-sm p-6 rounded-2xl shadow-[0_4px_14px_rgba(0,0,0,0.1)]">
              <h1 className="text-3xl font-bold text-orange-400">{data.title}</h1>

              {/* <p className="text-[#3a342f] leading-relaxed line-clamp-2">
        {data.description}
      </p>
       */}
              <p className='text-[#3a342f] leading-relaxed'>
                {showFull ? data.description : data.description.split(" ").slice(0, 20).join(" ") + "..."}
                <span
              ...
              <p className="text-xl font-semibold text-orange-400">
                Price: <span className="  font-bold text-[#3a342f]">${data.finalPrice}</span>
              </p>

              <p className="text-orange-400 font-semibold text-lg">
                Category: <span className="font-bold text-[#3a342f]">{data.category.name}</span>
              </p>
      ...
      {reviewError && (<ErrorMessage/>)}
       {reviewdata && reviewdata.length > 0 && (
         reviewdata.map((review)=>{
          const user = userdata?.find(user => user._id ===review.user._id);
          if(!user) return null;
          return(
           <ReviewCard key ={review._id} review={review} user={user} onDelete={(id)=>deletereview.mutate(id)}/>
          )
         })
        )}
        </>
       ...
       <ToggleButton isOpen={showReviews}
