...
const ReviewCard: React.FC<ReviewCardProps> = ({ review }) => {
    const { user, rate, comment, createdAt, doctor, service, reservation } = review;

    const renderStars = (rating: number) => {
        return [...Array(5)].map((_, index) => (
            <FaStar
                key={index}
                className={`w-4 h-4 ${index < rating ? "text-[#E5A46C]" : "text-[#EAE0D5]"
                    }`}
            />
        ));
    };
        ...
        if (reservation) {
            return (
                <div className="flex items-center gap-2 px-4 py-1.5 bg-[#F5EBE0] text-[#8C7A6B] rounded-full text-sm font-medium w-fit mt-4">
                    ...
                    <span>Reservation: {reservation.date}</span>
                </div>
            );
        }
        return (
        ...
        >
            <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="font-serif font-bold text-[#5A4033] text-lg">{user?.userName || "Anonymous"}</h3>
                        <p className="text-xs text-[#8C7A6B] font-medium tracking-wide uppercase">{formatDate(createdAt)}</p>
                    </div>
                </div>
