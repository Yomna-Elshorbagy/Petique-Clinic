...
import { getAllDoctors, type IUserDoctor } from "../../../Apis/DoctoresApis";
...
const ReviewModal: React.FC<ReviewModalProps> = ({ isOpen, onClose, onSuccess }) => {
    ...
    const [doctors, setDoctors] = useState<IUserDoctor[]>([]);
    const [services, setServices] = useState<IService[]>([]);

    const { register, handleSubmit, reset, setValue, watch, formState: { errors, isSubmitting } } = useForm<ReviewFormInputs>({
        defaultValues: {
            type: "clinic",
            rate: 0
        }
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                ...
                >
                    ...
                    <div className="p-6 border-b border-[#F0EAE0] flex justify-between items-center bg-gradient-to-r from-[#FDFBF7] to-white">
                        <div>
                            <h2 className="text-2xl font-serif font-bold text-[#5A4033]">Share Experience</h2>
                            <p className="text-sm text-[#8C7A6B]">We value your feedback</p>
                        </div>
                        <button
                            onClick={onClose}
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
                        ...
                        <div className="space-y-3">
                            <label className="text-sm font-medium text-[#5A4033] block">What are you reviewing?</label>
                            <div className="grid grid-cols-3 gap-3">
                                {[
                                    { id: "doctor", icon: Stethoscope, label: "Doctor" },
                                    { id: "service", icon: Syringe, label: "Service" },
                                    { id: "clinic", icon: Building2, label: "Clinic" },
                                ].map((type) => (
                                    <button
                                        key={type.id}
                                        type="button"
                                        ...
                                        className={`flex flex-col items-center justify-center gap-2 py-3 px-2 rounded-xl border transition-all duration-200 ${selectedType === type.id
                        ...
                        <AnimatePresence mode="wait">
                            {selectedType === "doctor" && (
                                ...
                                >
                                    <label className="text-sm font-medium text-[#5A4033]">Select Doctor</label>
                                    <select
                                        {...register("targetId", { required: selectedType === "doctor" })}
                                    ...
                                    {errors.targetId && <span className="text-red-500 text-xs">Please select a doctor</span>}
                                </motion.div>
                            )}
                            ...
                            {selectedType === "service" && (
                                <motion.div
                                    ...
                                    className="space-y-2 overflow-hidden"
                                >
                                    <label className="text-sm font-medium text-[#5A4033]">Select Service</label>
                                    <select
                                        {...register("targetId", { required: selectedType === "service" })}
                                        className="w-full p-3 rounded-xl border border-[#EAE0D5] focus:border-[#E5A46C] focus:ring-4 focus:ring-[#E5A46C]/10 outline-none transition-all bg-[#FDFBF7] focus:bg-white text-[#5A4033]"
                                    >
                                    ...
                                    {errors.targetId && <span className="text-red-500 text-xs">Please select a service</span>}
                                </motion.div>
                            )}
                        </AnimatePresence>
                        ...
                        <div className="space-y-3 text-center">
                            <label className="text-sm font-medium text-[#5A4033] block">Rate your experience</label>
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-medium text-[#5A4033]">Your Review</label>
                            <textarea
                            ...
                            <div className="flex justify-between text-xs text-[#D4C5B5] px-1">
                                <span className="text-red-500">{errors.comment?.message as string}</span>
