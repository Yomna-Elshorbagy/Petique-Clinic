...
const Sidebar = ({
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 z-30 h-screen w-64 bg-white dark:bg-[var(--color-dark-card)] border-r border-gray-200 dark:border-gray-800 transition-transform duration-300 ease-in-out transform lg:translate-x-0 ${
      ...
      >
        <div className="flex items-center gap-3 px-6 py-6 border-b border-gray-100 dark:border-gray-800">
          ...
          <span className="text-xl font-bold text-[var(--color-light-dark)] dark:text-[var(--color-dark-text)]">
            Petique
          </span>
        </div>
                ...
                <item.icon
                  className={`text-lg ${
                    isActive
                      ...
                      : "text-gray-400 group-hover:text-[var(--color-light-accent)]"
                  }`}
                />
                <span>{item.name}</span>
              </Link>
