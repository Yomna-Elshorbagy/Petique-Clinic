...
const DeletedCouponsTable: React.FC<DeletedCouponsTableProps> = ({
  searchCode,
}) => {
  ...
  const columns: TableColumn<ICoupon>[] = [
    ...
    },
    {
      name: "Deleted At",
      selector: (row: any) =>
        row.deletedAt ? new Date(row.deletedAt).toLocaleDateString() : "N/A",
      sortable: true,
      width: "140px",
    },
    {
  ...
  if (error) {
    return (
      <div className="p-4 text-red-500">Error loading archived coupons.</div>
    );
  }
