...
const StaffSidebar = () => {
  ...
  return (
    <div
      className={`h-screen bg-[#FCF9F4] border-r border-[#A98868]/20 transition-all duration-300 ease-in-out flex flex-col relative ${
    ...
    >
      ...
      <div className="p-6 flex items-center justify-center border-b border-[#A98868]/10">
        ...
        >
          <h1
            className={`font-bold text-2xl text-[#86654F] ${
          ...
          {isCollapsed && (
            <span className="text-xl font-bold text-[#C58D52]">P</span>
          )}
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-2">
        {navItems.map((item) => (
          ...
          >
            <item.icon
              size={22}
              className={`transition-colors min-w-[22px] ${
                isCollapsed ? "mx-auto" : ""
              }`}
            />
